...
const cargoModel = require('../model/addCargo')
const RouteModel = require('../model/RouteModel')
/* GET home page. */
router.get('/', async function(req, res, next) {
try {
  let jobs = await RouteModel.find();
...
router.get('/myBookings',  async(req,res)=>{
        try {
              ...
              var bookings = await cargoModel.find({userID:req.session.user._id});
...
router.post('/add-Cargo',async(req,res)=>{
      try {
        let userID = req.session.user._id;
            req.body.userID=userID
            let cargo = await cargoModel.create(req.body)
...
router.get('/recieved/:id',async(req,res)=>{
  try {
        let id = req.params.id;
        let cargo = await cargoModel.findByIdAndUpdate({_id:id},{$set:{status:"GotIT"}})
        res.redirect('/Home')
  } catch (error) {
        console.log(error)
  }
