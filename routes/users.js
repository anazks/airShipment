...
var cargoModel = require('../model/addCargo')
...
router.get('/home',async function(req, res, next) {
      try {
          ...
          let id = req.session.employee._id;
          console.log(id,"id emp")
          let empNme = req.session.employee.userName;
          let bookings = await bookingModel.find({employee:name});
          let AssignedWorkd = await cargoModel.find({boyId:id})
          console.log(AssignedWorkd,"works")
          res.render('employee/home',{bookings,AssignedWorkd})
...
router.post('/Login', async function(req, res, next) {
  try {
      ...
      let employee = await employeeModel.find({email:email,password:password,status:"Approved"})
...
router.get('/collect/:id',async(req,res)=>{
    try {
          ...
          let collect = await cargoModel.findByIdAndUpdate({_id:id},{$set:{status:"Collected"}})
          let Eid = req.session.employee._id;
          let BoyBussy = await employeeModel.findByIdAndUpdate({_id:Eid},{$set:{availableStatus:"bussy"}}) 
    } catch (error) {
